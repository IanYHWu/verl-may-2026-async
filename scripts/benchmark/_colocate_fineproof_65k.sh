#!/usr/bin/env bash
# Parameterized colocate benchmark for fineproof @ 65k. Override env vars:
#   MODEL_HUB_OR_PATH  — Qwen3.5-{4B,9B,27B} hub id or path (default 4B)
#   TP                 — tensor model parallel size (default 2)
#   PARAM_OFFLOAD/GRAD_OFFLOAD/OPTIMIZER_OFFLOAD — defaults all true (colocate competes with vLLM for KV)
