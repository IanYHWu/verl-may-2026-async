#!/usr/bin/env bash
# Parameterized async benchmark for fineproof @ 65k. Override env vars:
#   MODEL_HUB_OR_PATH  — Qwen3.5-{4B,9B,27B} hub id or path (default 4B)
#   MODE               — "mode1" or "mode4" (default mode4)
#   TP                 — tensor model parallel size (default 2)
#   N_GPUS_ROLLOUT     — rollout GPUs (default 4)
#   REQUIRE_BATCHES    — for mode4: trainer fetches REQUIRE_BATCHES * mini_bsz prompts per event
#   MINI_BSZ           — ppo mini batch size (default 32)
#   PARAM_OFFLOAD/GRAD_OFFLOAD/OPTIMIZER_OFFLOAD — defaults false
#   ROLLOUT_GMU        — vLLM gpu_memory_utilization (default 0.85, fineproof needs more KV)
#   N_CYCLES           — default 4
#   PROJECT_NAME, EXPERIMENT_NAME — required EXPERIMENT_NAME
...
MODEL_HUB_OR_PATH=${MODEL_HUB_OR_PATH:-/scratch/ssci-aviralku/huggingface/hub/models--Qwen--Qwen3.5-4B/snapshots/851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a}
MODE=${MODE:-mode4}
TP=${TP:-2}
...
TRAIN_FILE=${TRAIN_FILE:-"/scratch/schmidt/ssci-aviralku/ianwu/data/fineproof_dapo/train.parquet"}
