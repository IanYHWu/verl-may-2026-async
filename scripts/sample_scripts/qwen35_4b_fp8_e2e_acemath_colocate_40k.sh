#!/usr/bin/env bash
# B200 (SCHMIDT) colocate (hybrid) sync GRPO + Qwen3.5-4B + 40k responses +
# FP8 End-to-End.
#   - Training: FP8 hybrid via TE (e4m3 fwd / e5m2 bwd), delayed scaling.
#   - Optimizer: FP8 (precision-aware optim, delayed).
#   - Rollout:  vLLM FP8 blockwise.
#   - Dataset:  acemath_rl_4b_inst_hard (DAPO-formatted).
#   - Reward:   DAPO judge, 1.0 / 0.0 only, overlong penalty OFF.
#   - 60 training steps.
#   - TIS on. mtp_num_layers=null required for dense Qwen3.5.
#
# Defaults to FP8_RECIPE=delayed because CUDA 12.8 < 12.9 required by blockwise.
set -x
export CUDA_DEVICE_MAX_CONNECTIONS=1
export VLLM_USE_V1=1
export NCCL_IB_DISABLE=1

# Required by TE for FP8 block scaling (no-op for delayed but harmless).
...
max_response_length=${MAX_RESPONSE_LENGTH:-40960}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"qwen35_4b_fp8e2e_acemath_colocate_40k_bsz64_mbsz32"}

FP8_TRAIN_FORMAT=${FP8_TRAIN_FORMAT:-hybrid}      # hybrid or e4m3
FP8_RECIPE=${FP8_RECIPE:-delayed}                 # delayed | tensorwise | blockwise (cuda12.9+)
...
python -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.megatron.override_transformer_config.mtp_num_layers=null \
    +actor_rollout_ref.actor.megatron.override_transformer_config.fp8=${FP8_TRAIN_FORMAT} \
    +actor_rollout_ref.actor.megatron.override_transformer_config.fp8_recipe=${FP8_RECIPE} \
    +actor_rollout_ref.actor.optim.override_optimizer_config.optimizer_offload_fraction=1 \
    +actor_rollout_ref.actor.optim.override_optimizer_config.overlap_cpu_optimizer_d2h_h2d=True \
    +actor_rollout_ref.actor.optim.override_optimizer_config.use_precision_aware_optimizer=True \
    +actor_rollout_ref.actor.optim.override_optimizer_config.optimizer_cpu_offload=True \
    +actor_rollout_ref.actor.optim.override_optimizer_config.fp8_recipe=${FP8_RECIPE} \
