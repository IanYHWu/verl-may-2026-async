...
# BF16 EVERYWHERE. DEBUG run: 5 steps to validate chat template change.
#   - enable_thinking=False: pre-closes the <think> block in Qwen3.5's
#     chat template so the model emits a direct answer, not infinite
#     reasoning. Without this, Qwen3.5-4B never closes </think> on
#     acemath prompts and runs to the 40k cap.
#   - 0/1 reward (DAPO judge), no overlong penalty.
#   - TIS on.
#   - mtp_num_layers=null override needed for dense Qwen3.5.
set -x
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"qwen35_4b_bf16_acemath_colocate_40k_debug_nothink"}
