...
# FP8 rollout only.
#   - Training: BF16 (Megatron, mbridge legacy — Qwen3.5 only supported there).
#   - Rollout:  vLLM with FP8 blockwise quant.
#   - Dataset:  acemath_rl_4b_inst_hard (DAPO-formatted).
#   - Reward:   DAPO judge, 1.0 / 0.0 only, overlong penalty OFF.
#   - 60 training steps.
#   - TIS (rollout_is=token, threshold=2.0) kept on.
#   - mtp_num_layers=null is required for dense Qwen3.5 (see upstream_verl_port_notes.md).
set -x
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"qwen35_4b_fp8rollout_acemath_colocate_40k_bsz64_mbsz32"}
