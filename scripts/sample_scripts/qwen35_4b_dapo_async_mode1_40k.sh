#!/usr/bin/env bash
# B200 (SCHMIDT) Async Mode 1 (on-policy pipeline) + Qwen3.5-4B + 40k responses.
#   trigger_parameter_sync_step=1, staleness_threshold=0, partial_rollout=False
#
# Built from:
#   - scripts/qwen3_4b_inst_acemath_async_mode1.sh (Mode 1 async topology)
#   - scripts/qwen35_4b_32k_colocate.sh (Qwen3.5 hybrid-attention settings)
#   - docs/b200_qwen35_megatron_bringup.md
#
# Same hyperparams as the Mode 1 reference (clip 0.20/0.28, lr 1e-6, wd 0.01,
# ppo_mini_batch_size=32, rollout.n=8, require_batches=2). Topology is 4 trainer
# + 4 rollouter B200s; trainer DP=2 x TP=2; rollout = 2 vLLM servers each TP=2.
...
# Mode 1 cycle math: per cycle = 1 trigger * 2 require * 32 mini-bsz = 64 prompts
# (matches Mode 1 reference run on H100). 100 cycles -> 200 grad updates total.
...
# Length budget: 40k responses + 2k prompt + vLLM slack.
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"qwen35_4b_dapo_mode1_b200_40k_bsz64_mbsz32_clip028_lr1e6_wd001"}
