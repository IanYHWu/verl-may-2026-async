...
# responses + NVFP4 W4A16 QAT.
#   - Training: BF16 + ModelOpt fake quantization on Linear layers (W4A16).
#   - Rollout:  vLLM with packed NVFP4 (modelopt quant_method).
#   - Dataset:  acemath_rl_4b_inst_hard (DAPO-formatted).
#   - Reward:   DAPO judge (1.0 correct / 0.0 wrong, overlong penalty off).
...
#   - NVFP4 requires non-vanilla megatron-bridge.
set -x
export CUDA_DEVICE_MAX_CONNECTIONS=1
export VLLM_USE_V1=1
export NCCL_IB_DISABLE=1

# B200 has capability 10.0; vLLM would auto-pick FLASHINFER_CUTLASS, but the
# verl modelopt patch packs weights in Marlin layout. Force the Marlin backend
# so the forward path matches the weight packing.
export VLLM_NVFP4_GEMM_BACKEND=marlin

# TE attention backend debug (uncomment if backend-selection issues recur).
# export NVTE_DEBUG=1
# export NVTE_DEBUG_LEVEL=2

# Cudnn + nccl wheel libs on path (TE/cuDNN graph dlopen).
...
# BF16 init dequantized from OPENZEKA/Qwen3-4B-Instruct-2507-NVFP4. Starting
# from weights pre-rounded to the NVFP4 grid lets verl's on-the-fly W4 packing
# at vLLM init be near-lossless (vs the catastrophic collapse seen when
# starting from raw Qwen3-4B-Instruct-2507).
HF_MODEL_PATH=${HF_MODEL_PATH:-"/scratch/schmidt/ssci-aviralku/ianwu/models/Qwen3-4B-Instruct-2507-NVFP4-dequant-bf16"}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"qwen3_4b_inst_nvfp4_qat_acemath_colocate_16k_bsz64_mbsz32"}

CKPT_DIR=${CKPT_DIR:-"./checkpoints/${PROJECT_NAME}/${EXPERIMENT_NAME}"}

QAT_CONFIG_PATH=${QAT_CONFIG_PATH:-"${REPO_ROOT}/recipe/qat/config/nvfp4_w4a16_megatron.json"}
...
python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.megatron.qat.group_size=16 \
