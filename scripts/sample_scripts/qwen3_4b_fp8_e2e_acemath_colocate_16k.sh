...
# responses + FP8 End-to-End.
#   - Training: FP8 hybrid (e4m3 fwd / e5m2 bwd via TE), blockwise scaling.
#   - Optimizer: FP8 blockwise (precision-aware optim).
#   - Rollout: vLLM FP8 blockwise.
#   - Dataset:  acemath_rl_4b_inst_hard (DAPO-formatted).
#   - Reward:   DAPO judge (1.0 / 0.0, overlong penalty off).
#
# WARNING: Docs say FP8 blockwise needs CUDA 12.9+. We have CUDA 12.8.
# Override FP8_RECIPE=delayed (or tensorwise) if blockwise breaks.
set -x
export CUDA_DEVICE_MAX_CONNECTIONS=1
export VLLM_USE_V1=1
export NCCL_IB_DISABLE=1

# Required by Transformer Engine for FP8 block scaling.
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"qwen3_4b_inst_fp8e2e_acemath_colocate_16k_bsz64_mbsz32"}

FP8_TRAIN_FORMAT=${FP8_TRAIN_FORMAT:-hybrid}      # hybrid or e4m3
# Default to delayed (legacy TE recipe) because our CUDA is 12.8.
# blockwise requires CUDA 12.9+. Override via env to try blockwise/tensorwise.
FP8_RECIPE=${FP8_RECIPE:-delayed}                 # delayed | tensorwise | blockwise
