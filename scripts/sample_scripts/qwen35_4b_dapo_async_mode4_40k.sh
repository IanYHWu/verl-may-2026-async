...
# Qwen3.5-4B + 40k responses.
#   trigger_parameter_sync_step=4, staleness_threshold=0.5, partial_rollout=True,
#   require_batches=1 (streaming).
#
# Same hyperparams as the Mode 1 reference / Qwen3.5 Mode 1 40k counterpart
# (clip 0.20/0.28, lr 1e-6, wd 0.01, ppo_mini_batch_size=32, rollout.n=8,
# entropy_coeff=0, calculate_entropy=true). Topology: 4 trainer + 4 rollouter
# B200s; trainer DP=2 x TP=2; rollout = 2 vLLM servers each TP=2.
#
# Cycle math (matched to Mode 1 40k total grad updates):
#   Mode 1: N_CYCLES=50, trigger=1, require=2, mbsz=32 -> 100 grad updates.
#   Mode 4: trigger=4, require=1, mbsz=32 -> 4 grad updates per cycle.
#           N_CYCLES=25 -> 100 grad updates, 25*128=3200 prompts (same as Mode 1).
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"qwen35_4b_dapo_mode4_b200_40k_bsz32_trig4_stale05_partial_clip028_lr1e6_wd001"}
